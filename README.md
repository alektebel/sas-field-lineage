...
- **📄 Persistent-library table report**: download an Excel workbook of every table written to a persistent (non-`WORK`) SAS library — with `LIBNAME` paths/engines and the writing step (DATA, `PROC SQL CREATE TABLE/VIEW`, `OUT=`/`BASE=`) — resolving `%let`/`&macro` names, prefixes and `%do` loops first, and flagging any name it could not resolve
...
5. **Persistent-library table report** — the **Download persistent tables
   ...
   and its source line. `%let`/`&macro` names, prefixes and `%do` loops are
   resolved before reporting; anything still unresolved is kept as a flagged
   row instead of being dropped. The workbook also has a **Libraries** sheet
   and a **Summary** sheet.
