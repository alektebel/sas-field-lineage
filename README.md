...
- **⚠️ Parse warnings**: statements the parser cannot fully model are reported instead of silently dropped — unsupported DATA-step statements (`DROP`, `KEEP`, `IF`, …), unterminated `RUN;`/`QUIT;` steps, library-qualified (`lib.table`) targets, and unrecognized top-level statements. Shown in the explorer header, printed by the CLI (stderr) and added to the report's `Parser warnings` sheet
...
5. **Persistent-library table report** — the **Download persistent tables
   ...
   **Libraries** and **Summary** sheets.
6. **Parse warnings** — a `⚠ n parser warnings — lineage may be incomplete`
   indicator appears after loading a program with statements the parser cannot
   fully model (unsupported DATA-step statements, unterminated steps,
   library-qualified targets, unrecognized top-level statements). Hover it for
   the list; the same warnings are a sheet in the exported workbook.
7. **Pan & zoom** — drag anywhere on the canvas to roam the graph, scroll for
